...
def vr_interface(vrouter_ip, show_ns=False):
    ...
    res = requests.get(url='http://%s:8085/Snh_VrouterInfoReq' % vrouter_ip)
    ...
    res = requests.get(url='http://%s:8085/Snh_ItfReq' % vrouter_ip)
    if res.status_code != 200:
        print('error: host is not reachable')
        return
    res.encoding='utf8'
...
class parser_nodes():
    ...
    def cmd_delete(self, args):
        ...
        if not res_id:
            print('error: node %s not found' % args.hostname)
            return
        # delete node
        if args.type == 'vrouter':
            ...
            if vmi_id:
                self.res.url = "/virtual-machine-interface/%s" % vmi_id['uuid']
                self.action.set_res(self.res)
                self.action.delete()
        self.res.url = "/%s/%s" % (nodes_map[args.type], res_id['uuid'])
        self.action.set_res(self.res)
        self.action.delete()
...
class LoadBalanceMember(Resource):
    ...
    @pool.setter
    def pool(self, value):
        self.url = self.url.replace('member', 'pool/%s/member' % value)
